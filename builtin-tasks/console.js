...
const log = debug_1.default('redspot:core:tasks:console');
config_env_1.task(task_names_1.TASK_CONSOLE, 'Opens a redspot console')
    .addFlag('noCompile', "Don't compile before running this task")
    .setAction(async ({ noCompile }, { config, redspotArguments, run }) => {
    if (!noCompile) {
    ...
    log(`Creating a Node REPL subprocess with Redspot's register so we can set some Node's flags`);
    // Running the script "" is like running `node`, so this starts the repl
    await scripts_runner_1.runScriptWithRedspot(redspotArguments, '', [], nodeArgs, {
