...
function validateSourceNameFormat(sourceName) {
    if (isAbsolutePathSourceName(sourceName)) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.INVALID_SOURCE_NAME_ABSOLUTE_PATH, {
            name: sourceName
        });
    }
    if (isExplicitRelativePath(sourceName)) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.INVALID_SOURCE_NAME_RELATIVE_PATH, {
            name: sourceName
        });
    }
    ...
    if (replaceBackslashes(sourceName) !== sourceName) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.INVALID_SOURCE_NAME_BACKSLASHES, {
            name: sourceName
        });
    }
    if (normalizeSourceName(sourceName) !== sourceName) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.INVALID_SOURCE_NOT_NORMALIZED, {
            name: sourceName
        });
    }
}
...
async function isLocalSourceName(projectRoot, sourceName) {
    ...
    try {
        ...
        if (errors_1.RedspotError.isRedspotErrorType(error, errors_list_1.ERRORS.SOURCE_NAMES.FILE_NOT_FOUND)) {
            return false;
        }
...
async function validateSourceNameExistenceAndCasing(fromDir, sourceName) {
    ...
    if (trueCaseSourceName !== sourceName) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.WRONG_CASING, {
...
async function localPathToSourceName(projectRoot, localFileAbsolutePath) {
    ...
    if (normalized.startsWith('..')) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.EXTERNAL_AS_LOCAL, {
            path: localFileAbsolutePath
        });
    }
    if (normalized.includes(NODE_MODULES)) {
        throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.NODE_MODULES_AS_LOCAL, {
            path: localFileAbsolutePath
        });
    }
...
async function getPathTrueCase(fromDir, p) {
    ...
    try {
        ...
        if (typeof error.message === 'string' &&
            ...
            throw new errors_1.RedspotError(errors_list_1.ERRORS.SOURCE_NAMES.FILE_NOT_FOUND, {
                name: p
            }, error);
        }
        // tslint:disable-next-line only-redspot-error
