...
class HelpPrinter {
    constructor(_programName, _executableName, _version, _redspotParamDefinitions, _tasks) {
        this._programName = _programName;
        this._executableName = _executableName;
        this._version = _version;
        this._redspotParamDefinitions = _redspotParamDefinitions;
        this._tasks = _tasks;
    }
    ...
    _printParamDetails(paramDefinitions) {
        const paramsNameLength = Object.keys(paramDefinitions)
            ...
            .reduce((a, b) => Math.max(a, b), 0);
        for (const name of Object.keys(paramDefinitions).sort()) {
            const { defaultValue, description, isFlag, isOptional } = paramDefinitions[name];
