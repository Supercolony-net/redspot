...
class ArgumentsParser {
    ...
    static cLAToParamName(cLA) {
        if (cLA.toLowerCase() !== cLA) {
            throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.PARAM_NAME_INVALID_CASING, {
                param: cLA
            });
        }
        const parts = cLA
    ...
    parseRedspotArguments(redspotParamDefinitions, envVariableArguments, rawCLAs) {
        const redspotArguments = {};
        let taskName;
        const unparsedCLAs = [];
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (taskName === undefined) {
                ...
                if (!this._isCLAParamName(arg, redspotParamDefinitions)) {
                    throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_COMMAND_LINE_ARG, { argument: arg });
                }
                i = this._parseArgumentAt(rawCLAs, i, redspotParamDefinitions, redspotArguments);
            }
            else {
                if (!this._isCLAParamName(arg, redspotParamDefinitions)) {
        ...
        return {
            redspotArguments: this._addRedspotDefaultArguments(redspotParamDefinitions, envVariableArguments, redspotArguments),
            taskName,
            unparsedCLAs
        };
    }
    ...
    _parseTaskParamArguments(taskDefinition, rawCLAs) {
        ...
        for (let i = 0; i < rawCLAs.length; i++) {
            ...
            if (!this._isCLAParamName(arg, taskDefinition.paramDefinitions)) {
                throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_PARAM_NAME, {
                    param: arg
                });
            }
    ...
    _addRedspotDefaultArguments(redspotParamDefinitions, envVariableArguments, redspotArguments) {
        return {
    ...
    _parseArgumentAt(rawCLAs, index, paramDefinitions, parsedArguments) {
        ...
        if (parsedArguments[paramName] !== undefined) {
            throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.REPEATED_PARAM, {
                param: claArg
            });
        }
    ...
    _parsePositionalParamArgs(rawPositionalParamArgs, positionalParamDefinitions) {
        ...
        for (let i = 0; i < positionalParamDefinitions.length; i++) {
            ...
            if (rawArg === undefined) {
                if (!definition.isOptional) {
                    throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.MISSING_POSITIONAL_ARG, {
        ...
        if (!hasVariadicParam &&
            ...
            throw new errors_1.RedspotError(errors_list_1.ERRORS.ARGUMENTS.UNRECOGNIZED_POSITIONAL_ARG, {
